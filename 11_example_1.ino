...
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, dist_real, alpha; // unit: mm
...
// get a distance reading from the USS
  ...
  dist_real = USS_realmeasure(PIN_TRIG, PIN_ECHO);
...
myservo.writeMicroseconds(((_DUTY_MAX-_DUTY_MIN)/180)*(dist_ema-180)+_DUTY_MIN);
...
float USS_measure(int TRIG, int ECHO, int prevReading)
{
  float reading;
  ...
  if(reading == 0.0) {return prevReading;}
  else {return reading;}
}

float USS_realmeasure(int TRIG, int ECHO)
{
  float reading;
  digitalWrite(TRIG, HIGH);
  delayMicroseconds(10);
